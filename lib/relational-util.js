...
module.exports.makeent = function (ent, row) {
  ...
  var hints = row[SENECA_TYPE_COLUMN] || row[SENECA_TYPE_COLUMN.toUpperCase()]
  if (hints) {
  ...
  if (!_.isUndefined(ent) && !_.isUndefined(row)) {
    fields.forEach(function (field) {
      ...
      if (SENECA_TYPE_COLUMN != field) {
        if (_.isUndefined(senecatype[fieldl])) {
          entp[fieldl] = row[field]
        }
        else if (senecatype[fieldl] == OBJECT_TYPE) {
          entp[fieldl] = JSON.parse(row[field])
        }
        else if (senecatype[fieldl] == ARRAY_TYPE) {
          entp[fieldl] = JSON.parse(row[field])
        }
        else if (senecatype[fieldl] == DATE_TYPE) {
          entp[fieldl] = new Date(row[field])
        }
        else if (senecatype[fieldl] == BOOLEAN_TYPE) {
          entp[fieldl] = JSON.parse(row[field])
        }
      }
    })
