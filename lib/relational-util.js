...
var CHARACTER_TYPES = ['VARCHAR', 'NVARCHAR', 'ALPHANUM', 'SHORTTEXT'],
  DATETIME_TYPES = ['DATE', 'TIME', 'SECONDDATE', 'TIMESTAMP'],
  NUMERIC_TYPES = ['TINYINT', 'SMALLINT', 'INTEGER', 'BIGINT', 'SMALLDECIMAL', 'DECIMAL', 'REAL', 'DOUBLE'],
  BINARY_TYPES = ['VARBINARY'],
  LOB_TYPES = ['BLOB', 'CLOB', 'NCLOB', 'TEXT'],
// types allowed in where clause
  ALLOWED = _.union(CHARACTER_TYPES, DATETIME_TYPES, NUMERIC_TYPES);

var isAllowed = module.exports.isAllowed = function (dataType) {
  if (!dataType) {
    return false;
  }
...
module.exports.makeentp = function (ent, tblspec) {
  ...
  fields.forEach(function (field) {
    var cspec = tblspec.columns[field],
      dataType = (cspec && cspec.dataTypeName) ? cspec.dataTypeName : null,
      mappedval = getMapper(dataType).toSQL(ent[field]);
...
module.exports.makeent = function (ent, row, tblspec) {
  ...
  var entp = {},
    fields = _.keys(row),
    hints = row[SENECA_TYPE_COLUMN],
    senecatype = hints ? JSON.parse(hints) : {};
...
module.exports.tablename = function (entity) {
  var canon = entity.canon$({object: true}),
    name = (canon.base ? canon.base + '_' : '') + canon.name;
