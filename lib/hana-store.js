  ...
  var store = {
    ...
    save: function (args, cb) {
      ...
      getcoll(schema, ent, function (err, coll) {
        ...
        if (update) {
          dbinst.query(quoteprop(schema), updatestm(ent, coll), function (err, res) {
          ...
          dbinst.query(quoteprop(schema), idvalstm(coll.tableName), function (err, data) {
            if (!error(args, err, cb)) {
              var id = getid(data)
              if (!id) return cb(new Error('Error retrieving sequence value'))
              ent.id = id
              dbinst.query(quoteprop(schema), savestm(ent, coll), function (err, res) {
    ...
    load: function (args, cb) {
      ...
      getcoll(schema, qent, function (err, coll) {
        ...
        dbinst.query(quoteprop(schema), selectstm(qent, q, coll), function (err, res) {
          if (!error(args, err, cb)) {
