...
module.exports = function (opts) {
  ...
  function configure(spec, cb) {
    ...
    dbinst.connect(function (err) {
      ...
      dbinst.selectSchema(conf.schema, function (err, session) {
        if (err) {
          console.log('Error setting schema: %s', util.inspect(err))
        } else {
          console.log('Schema set to: %s, %s', schema, session)
        }
      })
      return cb()
    });
  ...
  var store = {
    ...
    remove: function (args, cb) {
      ...
      if (q.all$) {
        ...
        dbinst.query(schema, selectstm(qent, q), function (err, res) {
          if (!error(args, err, cb)) {
            var entp = res.rows[0]
...
function savestm(ent) {
  ...
  fields.forEach(function (field) {
    cols.push(field.toUpperCase())
    vals.push(quoteval(entp[field]))
  })

  return 'INSERT INTO ' + quoteprop(table.toUpperCase()) + ' (' + cols + ') VALUES (' + vals + ')'
}
...
function updatestm(ent) {
  ...
  fields.forEach(function (field) {
    if (!(_.isUndefined(ent[field]) || _.isNull(ent[field]))) {
      params.push(field.toUpperCase() + '=' + quoteval(entp[field]))
    }
  })

  return 'UPDATE ' + quoteprop(table.toUpperCase()) + ' SET ' + params + ' WHERE id=' + quoteval(ent.id)
}
...
function deletestm(qent, q) {
  ...
  return 'DELETE FROM ' + quoteprop(table.toUpperCase()) + wherestm(entp, q)
...
function selectstm(qent, q) {
  ...
  return 'SELECT * FROM ' + quoteprop(table.toUpperCase()) + wherestm(entp, q) + metastm(q)
}
...
function wherestm(entp, q) {
  ...
  for (var p in qok) {
    if (qok[p]) {
      wargs.push(p.toUpperCase() + '=' + quoteval(qok[p]))
    }
  }
...
function metastm(q) {
  ...
  if (q.sort$) {
    ...
    params.push('ORDER BY ' + sf.toUpperCase() + ' ' + sd)
  }
