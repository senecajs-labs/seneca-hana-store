...
function inittblmap(dbinst, schema, cb) {
  ...
  var query = 'SELECT SCHEMA_NAME,TABLE_NAME,COLUMN_NAME,DATA_TYPE_ID,DATA_TYPE_NAME,LENGTH,SCALE,IS_NULLABLE,MIN_VALUE,MAX_VALUE FROM "SYS"."COLUMNS" WHERE "SCHEMA_NAME" = ' + quoteval(schema)
    ...
    if (coldata.rows) {
      coldata.rows.forEach(function (row) {
        ...
        if(tblname.indexOf('::')!=-1){
          tblname = tblname.substr(tblname.indexOf('::')+2)
        }
        if (!collmap[tblname]) {
          collmap[tblname] = {
